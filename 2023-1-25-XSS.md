...
//What the code does here is store the value of the query in the "store" variable, and later check its value in the if. If the result is true (ie, the storeId exists), go into document.write and write the store+/option. Therefore, we can inject a payload into the StoreId field, since it is not sanitized. Therefore, it is enough to add this code to the url:
//&storeId=<script>alert(2)</script>
```
## DOM XSS in AngularJS expression with angle brackets and double quotes HTML-encoded
```js
// As AngularJs scripts are executed using {{}}, we see that it is prone to SSTI type injections. In addition to this, the ng-app attribute is used, so we put a payload in it with alert():
{{this.constructor.constructor('alert("lo_que_sea")')()}}}
// The way this works is simple:
// If we put a script in Angular as for example can be {{alert(1)}}, the Angular sandboxing will block it, therefore, we need to escape from this sandbox somehow to be able to execute malicious code. Therefore, since we are dealing with a vulnerable ng-app, we can use the attribute 
...
// Looking at its source code, we are going to look for .innerHTML elements (knowing that it is a DOM), and we find the following function:
function escapeHTML(html) {
    return html.replace('<', '&lt;').replace('>', '&gt;');
}
// Which replaces the <>. However, when the first argument is a string, it simply replaces the first <> found, leaving the rest untouched, so we can put a payload like this in it:
<><img src=1 onerror=alert(1)>
...
```js
// Since the victim user will see any comment posted, we need to create a comment that will remove the cookie and send it to us (via the burp collaborator).
...
//With this script, we manage to send the user and password fields (when this is changed) to our burp collaborator server whenever a person sees our post.
...
```js
// We intercept a search request with Burp, we kill it to the repeater and we put <test> inside the search field, where test will be the field to brute-force in search of the tags that by-pass the WAF. We find that body is a valid tag. Now it's time to look for an event to inject the print() command.
// Then we add <body%20a=> to the search field of the intruder, where 'a' will be the field to brute-force in the event search. We see that onresize is a valid event with the body tag, so we are going to create an exploit that we will send to the victim:
<iframe src="https://YOUR-LAB-ID.web-security-academy.net/?search=<body onresize=print()>" onload=this.style.width='100px'> // We have to add "onload" so that the "onresize" event is activated when the page of the <iframe> is loaded
//URL ENCODE:
...
// Now it's time to brute-force the event. And we see that we only have one valid:
onbegin
...
var searchTerms = 'test\'payload'; // puts the \ to prevent us from going outside the current string. However, we are going to try to close tags if we put the corresponding syntax. Which if he lets us do.
// So, we can close the javascript we're in by just putting </script> in the search bar and then declaring our own script to output an alert(1) per screen, so the syntax for solve the lab would be this:
</script><script>alert("whatever")</script>
```
## Stored XSS into onclick event with angle brackets and double quotes HTML-encoded and single quotes and backslash escaped
```js
// We see that our input from the website field (which is the one we are interested in infecting) is stored within an <onclick> tag, so it will be executed when clicked.
...
// As data, the javascript templates allow the execution of expressions inside them (that is, if we put an alert between the ``, it will be displayed on the screen)
...
// In it, we can see that it creates an alternative to href, but using <svg> and its "animate" event. However, your code contains words blocked by our WAF, so you have to look for an alternative:
...
<animate attributeName> // We indicate the name of the CSS property or attribute ("text in our case") of the target element that is going to be changed during an animation (that's why we use animate). In addition, we have to put =href, since it is the attribute that we are interested in using for redirects
href values=javascript:alert(1) // What will happen when we click.
<text x=20 y=20>Click me</text> /// We simply define the appearance of the "Click Me" text. In order to give it shape, we use the <text></text> tags as they are the ones that the page allows us to do.
```
